...
const Main = () => {
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    try {
      const res = await axiosServices.post("/api/tasks/create-task", data, {
        headers: {
          token: JWT,
          Accept: "application/json",
          "Content-Type": "application/json",
  ...
  const handleDelete = async (task_data) => {
    try {
      const res = await axiosServices.delete("/api/tasks/delete", {
        data: task_data,
      });
      if (res.status == 200) {
        await fetchTasks();
  ...
  const handleUpdate = async (task_data) => {
    try {
      const res = await axiosServices.post("/api/tasks/edit", {
        id: task_data.id,
        userId: task_data.userId,
        taskTitle: task_data.taskTitle,
        taskDetail: task_data.taskDetail,
